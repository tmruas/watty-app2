...
export default function LoginWizard({ disabled, args }: ComponentProps) {
  const wattyArgs = (args ?? {}) as WattyArgs;
  ...
  const bumpFrameHeight = useCallback(() => {
    ...
    const h = Math.max(fromShell, fromRoot, fromDoc, viewFloor, 320);
    Streamlit.setFrameHeight(h);
  }, []);

  const publicAsset = useCallback(
    (name: string) => `${import.meta.env.BASE_URL}${name}`,
    []
  );
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ...
      Streamlit.setComponentValue(pendingAuth.current);
    }, LOADING_MS);
    ...
    >
      {configMissing && (
        <div className="mx-auto max-w-lg px-4 py-6 text-center text-sm font-semibold text-red-700">
          Falta configurar o Supabase nesta sessão (URL e chave anon no servidor Streamlit).
        </div>
      )}
