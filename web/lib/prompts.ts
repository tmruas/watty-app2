import { IAAVE_DEFAULT, IAAVE_STRUCTURES } from "./iaave";
...
export function buildChatSystemPrompt(
  ...
  return `És o Watty, um tutor especialista em ${disciplina} para o ${ano} em Portugal.
...
export function buildExamPrompt(
  ...
  const estruturaOficial =
    IAAVE_STRUCTURES[disciplina] ?? IAAVE_DEFAULT;

  return `
                        Ages como o IAVE (Instituto de Avaliação Educativa) a criar o EXAME FINAL SIMULADO de ${disciplina} para o ${ano} em Portugal.
                        Temas a avaliar: ${temasExame}.
...
export function cicloExameGlobal(
  disciplina: string,
  ano: string,
): string {
  if (["Português", "Matemática A", "História A"].includes(disciplina)) {
    return "10º, 11º e 12º anos";
  }
  if (ano === "8º Ano" || ano === "9º Ano") {
    return "todo o 3º Ciclo (7º, 8º e 9º anos)";
  }
  return "10º e 11º anos";
}
