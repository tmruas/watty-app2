import { REGRA_GRAFICOS } from "./prompts";

const ESTRUTURAS_IAVE: Record<string, string> = {
  Economia: `
...
export function buildPromptExame(
  disciplina: string,
  ano: string,
  temasExame: string
): string {
  const estruturaOficial =
    ESTRUTURAS_IAVE[disciplina] ?? ESTRUTURA_DEFAULT;
  return `
