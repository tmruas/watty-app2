...
# 🏋️ ABA QUIZZES (COM BOSS BATTLE)
    ...
    if btn_gerar and tema_exercicios:
        with st.spinner("O Watty está a forjar o teu nível... 🛠️"):
            ...
            """
            try:
                resposta = client.models.generate_content(model='gemini-2.5-flash', contents=prompt_treino)
                ...
                st.session_state.quiz_pontos_max = sum(q.get("pontos", 10) for q in quiz_data)
                            ...
                            with st.spinner("🧠 O Watty está a corrigir a tua resposta..."):
                                ...
                                try:
                                    resp_correcao = client.models.generate_content(
                                        model='gemini-2.5-flash', contents=prompt_correcao
                                    )
